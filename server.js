...
app.get('/api/admin/all-tasks', (req, res) => {
    db.all(
        `SELECT t.*, u.username as employer_name, 
         (SELECT COUNT(*) FROM applications WHERE task_id = t.id) as applications_count,
         (SELECT COUNT(*) FROM applications WHERE task_id = t.id AND status = 'accepted') as accepted_count
         FROM tasks t
         JOIN users u ON t.employer_id = u.id
...
app.delete('/api/admin/delete-task/:taskId', (req, res) => {
    const taskId = req.params.taskId;

    // Удаляем БЕЗ возврата денег - наказание за нарушение правил!
    db.run('DELETE FROM applications WHERE task_id = ?', [taskId], (err) => {
        if (err) {
            return res.status(500).json({ error: 'Ошибка удаления откликов' });
        }

        db.run('DELETE FROM chat_messages WHERE task_id = ?', [taskId], (err) => {
            if (err) {
            ...
            db.run('DELETE FROM tasks WHERE id = ?', [taskId], (err) => {
                if (err) {
                    return res.status(500).json({ error: 'Ошибка удаления задания' });
                }

                res.json({ message: 'Задание удалено. Я баллы НЕ возвращены.' });
            });
        });
    });
});
...
// Отмена отклика администратором (работник исчез)
app.post('/api/admin/cancel-application', (req, res) => {
    const { applicationId, reason } = req.body;

    if (!applicationId) {
        return res.status(400).json({ error: 'applicationId обязателен' });
    }
    ...
    db.get(
        ...
        (err, application) => {
            ...
            if (application.status !== 'accepted') {
                return res.status(400).json({ error: 'Отклик не принят' });
            }

            // 1. Отменяем отклик
            db.run('UPDATE applications SET status = "cancelled" WHERE id = ?', [applicationId], (err) => {
                if (err) {
                    return res.status(500).json({ error: 'Ошибка отмены отклика' });
                }

                // 2. Возвращаем задание в статус "open"
                db.run('UPDATE tasks SET status = "open" WHERE id = ?', [application.task_id], (err) => {
                    if (err) {
                        return res.status(500).json({ error: 'Ошибка обновления задания' });
                    }

                    // 3. Снижаем рейтинг работника на 1.0
                    db.run(
                        'UPDATE users SET rating = MAX(0, rating - 1.0) WHERE id = ?',
                        [application.worker_id],
                        (err) => {
                            if (err) {
                                return res.status(500).json({ error: 'Ошибка снижения рейтинга' });
                            }

                            res.json({ 
                                message: 'Отклик отменён. Задание снова открыто. Рейтинг работника снижен на 1.0',
                                refunded: application.payment
                            });
                        }
                    );
                });
